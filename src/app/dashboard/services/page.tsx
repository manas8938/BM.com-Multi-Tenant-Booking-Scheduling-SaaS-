    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-stone-500 dark:text-stone-400 mt-0.5">Set pricing and duration for each service</p>
        </div>
        {!showForm && (
        ...
        <div className={`bg-white dark:bg-ink-800 rounded-xl border shadow-sm p-6 ${editItem ? 'border-ember-200 dark:border-ember-500/40' : 'border-stone-200 dark:border-ink-700'}`}>
          ...
          <form action={editItem ? updateService : createService} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1.5">
                  Price <span className="text-stone-400 dark:text-stone-500 font-normal">(USD)</span>
                </label>
      ...
      <div className="bg-white dark:bg-ink-800 rounded-xl border border-stone-200 dark:border-ink-700 shadow-sm overflow-hidden">
        {!services?.length ? (
          <div className="flex flex-col items-center justify-center py-16 text-center px-4">
            ...
            <p className="font-semibold text-stone-700 dark:text-stone-300">No services yet</p>
            <p className="text-sm text-stone-400 dark:text-stone-500 mt-1 mb-5">Add the services your business offers</p>
            <Link href="?new=1" className={BTN_PRIMARY}>
              <Plus size={16} />
              Add Service
            </Link>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-stone-100 dark:border-ink-700 bg-stone-50/80 dark:bg-ink-900/40">
                <th className="text-left text-xs font-semibold text-stone-500 dark:text-stone-400 uppercase tracking-wide px-5 py-3.5">Service</th>
                <th className="text-left text-xs font-semibold text-stone-500 dark:text-stone-400 uppercase tracking-wide px-5 py-3.5">Duration</th>
                <th className="text-left text-xs font-semibold text-stone-500 dark:text-stone-400 uppercase tracking-wide px-5 py-3.5">Price</th>
