...
export default async function BranchesPage({
  ...
  const editItem = searchParams.edit
    ? branches?.find((b) => b.id === searchParams.edit)
    : null
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-stone-900 dark:text-white">Branches</h1>
          <p className="text-sm text-stone-500 dark:text-stone-400 mt-0.5">Manage your business locations</p>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <div className={`bg-white dark:bg-ink-800 rounded-xl border shadow-sm p-6 ${editItem ? 'border-ember-200 dark:border-ember-500/40' : 'border-stone-200 dark:border-ink-700'}`}>
          ...
          <form action={editItem ? updateBranch : createBranch} className="space-y-4">
            {editItem && <input type="hidden" name="id" value={editItem.id} />}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1.5">Branch name</label>
                <input name="name" type="text" required placeholder="Main Branch" defaultValue={editItem?.name ?? ''} className={INPUT} />
              </div>
              <div>
                <label className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1.5">Address</label>
                <input name="address" type="text" required placeholder="123 Main St, City" defaultValue={editItem?.address ?? ''} className={INPUT} />
              </div>
            </div>
            <div className="flex items-center gap-3 pt-2">
              <div className="w-auto">
                <SubmitButton loadingText="Saving...">{editItem ? 'Save Changes' : 'Create Branch'}</SubmitButton>
              </div>
              <Link href="/dashboard/branches" className={BTN_SECONDARY}>Cancel</Link>
            </div>
          </form>
        </div>
      )}
        ...
        {!branches?.length ? (
          <div className="flex flex-col items-center justify-center py-16 text-center px-4">
            <div className="w-14 h-14 bg-ember-50 dark:bg-ember-500/20 rounded-full flex items-center justify-center mb-4">
              <GitBranch size={24} className="text-ember-400" />
            </div>
            <p className="font-semibold text-stone-700 dark:text-stone-300">No branches yet</p>
            <p className="text-sm text-stone-400 dark:text-stone-500 mt-1 mb-5">Add your first location to get started</p>
            <Link href="?new=1" className={BTN_PRIMARY}>
              <Plus size={16} />
              Add Branch
            </Link>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-stone-100 dark:border-ink-700 bg-stone-50/80 dark:bg-ink-900/40">
                ...
                <th className="text-left text-xs font-semibold text-stone-500 dark:text-stone-400 uppercase tracking-wide px-5 py-3.5">Address</th>
                <th className="px-5 py-3.5 w-24" />
              </tr>
            </thead>
            <tbody className="divide-y divide-stone-100 dark:divide-ink-700">
              {branches.map((branch) => (
                <tr key={branch.id} className="hover:bg-stone-50/60 dark:hover:bg-white/5 transition-colors">
                  <td className="px-5 py-3.5 font-medium text-stone-900 dark:text-white">{branch.name}</td>
                  <td className="px-5 py-3.5 text-stone-500 dark:text-stone-400">{branch.address}</td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center justify-end gap-1">
                      <Link href={`?edit=${branch.id}`} className="p-1.5 rounded-lg text-stone-400 dark:text-stone-500 hover:text-ember-600 hover:bg-ember-50 dark:hover:bg-ember-500/20 transition-colors" title="Edit">
                        <Pencil size={14} />
                      </Link>
                      <form action={deleteBranch}>
                        <input type="hidden" name="id" value={branch.id} />
                        <DeleteButton confirm={`Delete "${branch.name}"?`} />
                      </form>
                    </div>
                  </td>
                </tr>
