    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-0.5">Manage your team members</p>
        </div>
        {!showForm && (
        ...
        <div className={`bg-white rounded-xl border shadow-sm p-6 ${editItem ? 'border-ember-200' : 'border-stone-200'}`}>
          ...
          <form action={editItem ? updateStaff : createStaff} className="space-y-4">
            ...
            <div className="max-w-xs">
              <label className="block text-sm font-medium text-stone-700 mb-1.5">Branch <span className="text-stone-400 font-normal">(optional)</span></label>
      ...
      <div className="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden">
        {!staffList?.length ? (
          <div className="flex flex-col items-center justify-center py-16 text-center px-4">
            ...
            <p className="text-sm text-stone-400 mt-1 mb-5">Add your first team member to start scheduling</p>
            <Link href="?new=1" className={BTN_PRIMARY}>
              <Plus size={16} />
              Add Staff Member
            </Link>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-stone-100 bg-stone-50/80">
                ...
                <th className="text-left text-xs font-semibold text-stone-500 uppercase tracking-wide px-5 py-3.5">Email</th>
                <th className="text-left text-xs font-semibold text-stone-500 uppercase tracking-wide px-5 py-3.5">Branch</th>
            ...
            <tbody className="divide-y divide-stone-100">
              {staffList.map((member) => (
                <tr key={member.id} className="hover:bg-stone-50/60 transition-colors">
                  <td className="px-5 py-3.5 font-medium text-stone-900">{member.name}</td>
                  <td className="px-5 py-3.5 text-stone-500">{member.email}</td>
                  <td className="px-5 py-3.5 text-stone-500">
                    {(member.branches as { name: string } | null)?.name ?? <span className="text-stone-300">—</span>}
                  </td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center justify-end gap-1">
                      <Link href={`?edit=${member.id}`} className="p-1.5 rounded-lg text-stone-400 hover:text-ember-600 hover:bg-ember-50 transition-colors" title="Edit">
                        <Pencil size={14} />
                      </Link>
